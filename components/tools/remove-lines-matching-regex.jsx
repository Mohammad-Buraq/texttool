...
// v5
export default function RemoveLinesMatchingRegex(){
  const [input,setInput]=useState('Info\nDEBUG details\nInfo2\ndebug lower')
  const [pattern,setPattern]=useState('DEBUG')
  const [flags,setFlags]=useState('i')
  let output=''
  try{
    const fl=(flags||'').replace(/g/g,'')
    const re = pattern ? new RegExp(pattern, fl) : null
    const kept = re ? (input||'').split(/\r?\n/).filter(l=>{ re.lastIndex=0; return !re.test(l) }) : (input||'').split(/\r?\n/)
    output = kept.join('\n')
        ...
        <div>
          ...
          <div className="mt-2 flex gap-4 items-center flex-wrap">
            <label className="flex items-center gap-2">Pattern
              <input value={pattern} onChange={e=>setPattern(e.target.value)} placeholder="DEBUG (v5)" />
            </label>
